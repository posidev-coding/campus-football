...
describe('what these endpoints cannot do, which is the specification', function () {
    ...
    it('has no create, no delete and no arbitrary status', function () {
        ...
        // And there is no route on `/ops/issues/{issue}` at all — every write
        // is named after a transition, so the reachable set is bounded by the
        // routing table rather than by a validator somebody widens.
