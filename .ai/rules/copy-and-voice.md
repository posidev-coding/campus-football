...
The verify callout is its own tiny Livewire component (`livewire/verify-callout`) precisely so its ambient `wire:poll.30s` re-renders the row alone — as a Blade component the poll bound to the HOST root and re-rendered the whole screen for every unverified reader. Its guard is still the component's own `@if (unverified)`: the row, its poll and its work all cease on the verified render. Two couplings must survive any edit: the flip dispatches `email-verified` and every embed forwards it to the host as ONE `$refresh` (the wallet chips live in the host tree); and `.visible` stays OMITTED on purpose — dismissal `display:none`s the row via x-show, and the flip must still reach a reader who waved the nudge away. Don't "fix" either by adding a guard or the modifier. The notice screen's `wire:poll.10s` is licensed by `mount()` redirecting verified visitors — the screen only exists while there is something to poll.
...
Slate deadline and official-final resolve against a WEEK's own Saturday via App\Support\Cadence, overridable from the Pick'em Settings Filament page (pickem_settings, one nullable-override row — the brand pattern). The shipped defaults are Cadence's OWN constants (DEADLINE_* / OFFICIAL_*: Thursday noon and Sunday noon ET since 2026-08-20 — an earlier version of this rule restated "Tue 23:59:59" and was itself stale). Any copy or doc that names the clock reads `Cadence::deadlineLabel()` / `officialLabel()`, never a weekday: the laws partial said "due Tuesday night" for two weeks after the move, and InviteTemplates is the pattern that got it right. Everything is Eastern wall time: travelTo() in tests speaks UTC, and 01:00 UTC Wednesday is still Tuesday night in Knoxville — a cadence test that forgets this passes the wrong branch. The slate window is Game::inSlateWindow() (Saturday, ET hour >= 12) — the time-of-day half CANNOT be asked in SQL (DST shifts the UTC boundary mid-season), so it is a per-game PHP check layered on the kickoff_day scope. pickem:publish-slates sweeps hourly past the deadline and publishes the standard slate through AutoPublishStandardSlate → PublishSlate::force (same validation, no actor gates).
