...
The installed app has no back button, address bar or reload, so exits are product surface, not decoration. The auth layout's Back rides `window.cfbAppDepth` (defined in partials/head, shared so a cold load on EITHER layout counts); every errors/* page is self-contained (Brand reads rescue()d to constants, static PG copy) and ends in a home link or reload. Pull-to-refresh (components/pull-to-refresh) engages only on BOTH standalone signals, stays passive/never preventDefault(), axis-locks away from the swiper, and hands over a real location.reload() — whose spinner puck is the WHOLE refresh experience: the boot-splash stamp skips `reload` navigations, so no curtain plays on a pull. The viewport meta is zoom-locked (`maximum-scale=1, user-scalable=no` + `touch-action: manipulation`) because iOS focus auto-zoom left standalone permanently enlarged and side-scrolling — never loosen it to answer a zoom complaint. PwaTest pins all of this; long-form why in docs/ui-system.md.

## The boot splash is stylesheet-shown pre-Alpine and must self-clear without JS
`data-boot` is stamped by a pre-paint head script ONLY when `window.cfbAppDepth === undefined` (real document loads — the depth counter already exists on any hop re-evaluation), a standalone signal is present, AND the navigation type is not `reload` (`performance.getEntriesByType('navigation')[0]?.type` — a reload in standalone is a near-exact proxy for pull-to-refresh, whose puck is that gesture's whole experience; the `?.` fails OPEN to a launch); the stylesheet displays `[data-boot-splash]` under it, `end()` un-stamps at ~2.7s, and the 8s `cfb-boot-bail` CSS animation is the dead-man for dead JS. Never gate the curtain on x-show/x-cloak alone, and never rely on a mid-session stamp surviving `Livewire.navigate` — it refreshes `<html>` attributes (a console repro drives `begin()` directly). `data-standalone-only` is the deliberate mirror of `data-install-only` and HIDES when neither signal is present rather than forcing a display type, so any layout can wear it. BootSplashTest pins the timings.

## A Blade directive in a COMPONENT TAG's attribute never compiles
`<x-filament::button x-on:click="w(@js($payload))">` ships the LITERAL string `@js($payload)` to the browser: Blade compiles directives in ordinary template text, but a component tag's static attribute value is captured verbatim into the attribute bag. Alpine then cannot parse it and the handler is INERT — no console error, no exception. Same family as "an Alpine expression that starts with a comment never runs". A plain `<button x-on:click="…@js()…">` compiles fine, which is why one copy button worked and the one beside it did not (workbook hand-off modal, 2026-08-30).

Compose the expression in PHP with `Illuminate\Support\Js::from()` and pass it BOUND (`:x-on:click="$handler"`), the way the Workbook page's header action already does. Assert the RENDERED attribute — `navigator.clipboard` needs a secure context and does not exist in the automated tab — and note that a Filament modal's body is NOT in the page component's `html()`, so render the partial directly. WorkbookBoardTest sweeps every blade for `/<x-[^>]*@(js|json)\(/s`.

## Never key an Alpine scope to a Blade conditional inside a tag
`@if` wrapping whole ELEMENTS is fine — the scope and its readers appear and disappear together. Opening the conditional INSIDE a tag is different: the element renders either way and only its Alpine scope is keyed by the prop, so one component ships two scopes. The verify callout did exactly this (`x-data`, `x-show`, `x-cloak` all inside `@if ($dismissable)`), so `dismissed` existed on Home and Account and nowhere on the five picks surfaces. Alpine reports the miss as a bare ReferenceError from its evaluator with no element and no file attached, so it lands against whatever path the reader is standing on — production saw `Can't find variable: dismissed` on /verify-email, a screen with no callout on it. Attach the scope UNCONDITIONALLY and put only the readers (x-show, the dismiss handler) behind the flag; where a METHOD must stay conditional, condition it inside the x-data literal so the state stays unconditional (pick-card's `optimistic()`). `AlpineExpressionsTest` sweeps every Blade for the shape.
